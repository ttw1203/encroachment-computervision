"""Enhanced TTC event processing with 5-stage filtering pipeline for robust collision detection."""
...
class EnhancedTTCProcessor:
    """
    Enhanced TTC processor with 5-stage filtering pipeline:
    1. Hysteresis Logic - Prevents toggling with separate ON/OFF thresholds
    2. Persistence Threshold - Requires sustained conditions over multiple frames
    3. Confidence-Based Filtering - Skips low-confidence detections
    4. Relative Angle Filtering - Validates approach angles
    5. Vehicle Dimension Collision - AABB overlap prediction using real dimensions
    """

    def __init__(self, config):
        """Initialize enhanced TTC processor with configuration."""
        self.config = config
        self.video_fps = config.get('video_fps', 30.0)

        # TTC Configuration (with defaults)
        ...
        self.ttc_min_relative_angle = config.get('TTC_MIN_RELATIVE_ANGLE', 10)  # degrees
        self.ttc_max_relative_angle = config.get('TTC_MAX_RELATIVE_ANGLE', 150)  # degrees
        self.cleanup_timeout_frames = config.get('TTC_CLEANUP_TIMEOUT_FRAMES', 90)

        # Vehicle dimensions for AABB collision detection (length, width in meters)
        ...
        self.ttc_event_history: Dict[str, int] = {}  # event_id -> last_frame
    ...
    def process_ttc_events(self, detections, kf_states: Dict, frame_idx: int,
        ...
        """
        Main TTC processing pipeline with 5-stage filtering.

        Args:
            detections: Current frame detections with confidence scores
            kf_states: Kalman filter states for all tracked objects
            frame_idx: Current frame index
            detector_tracker: Detection tracker for class name lookup

        Returns:
            List of validated TTC events
        """
        current_events = []
        ...
        for i, tracker_i in enumerate(active_trackers):
            for j, tracker_j in enumerate(active_trackers[i+1:], i+1):
                ...
                # Apply 5-stage filtering pipeline
    ...
    def _apply_ttc_filters(self, pair_key: Tuple[int, int], kf_states: Dict,
        ...
        """Apply the 5-stage TTC filtering pipeline."""
        ...
        # Extract state vectors (x, y, vx, vy) in real-world coordinates (meters, m/s)
        ...
        rx0, ry0 = xj - xi, yj - yi  # Relative position
        vx, vy = vxj - vxi, vyj - vyi  # Relative velocity
    ...
    def _apply_hysteresis_filter(self, pair_key: Tuple[int, int],
        ...
        """
        FILTER 1: Hysteresis Logic
        Prevents rapid toggling by using separate ON/OFF thresholds.
        """
        current_active = self.ttc_active_states.get(pair_key, False)

        if not current_active:
            # Currently inactive - check ON conditions
            ...
            # Currently active - check OFF conditions
    ...
    def _apply_persistence_filter(self, pair_key: Tuple[int, int], frame_idx: int) -> bool:
        """
        FILTER 2: Persistence Threshold
        Requires sustained conditions over multiple frames.
        """
    ...
    def _apply_confidence_filter(self, conf_i: float, conf_j: float) -> bool:
        """
        FILTER 3: Confidence-Based Filtering
        Skips TTC evaluation if either detection has low confidence.
        """
    ...
    def _apply_relative_angle_filter(self, vxi: float, vyi: float,
        ...
        """
        FILTER 4: Relative Angle Filtering
        Validates approach angles between vehicles.

        Returns:
            Relative angle in degrees if valid, None if filtered out
        """
        # Skip if very close (emergency override)
        if d_closest < 0.5:
            return 0.0  # Allow regardless of angle

        # Calculate velocity magnitudes
        speed_i = math.hypot(vxi, vyi)
        speed_j = math.hypot(vxj, vyj)

        # Skip if either vehicle is stationary
        ...
        # Calculate angle between velocity vectors (0-180 degrees)
        ...
        # Filter based on angle constraints
    ...
    def _apply_vehicle_dimension_filter(self, tracker_i: int, tracker_j: int,
        ...
        # Construct AABB boxes (axis-aligned bounding boxes)
    ...
    def _cleanup_inactive_pairs(self, current_frame: int) -> None:
        ...
        Removes pairs where either tracker hasn't been seen recently.
        """
        pairs_to_remove = []
    ...
    def get_debug_info(self) -> Dict:
        """Get debug information about current TTC state."""
    ...
    def export_events_for_csv(self) -> List[List]:
        """Export validated TTC events in CSV format."""
        csv_rows = []
...
# Legacy compatibility wrapper for existing EventProcessor
class EventProcessor:
    """
    Enhanced EventProcessor with TTC filtering pipeline.
    ...
    def __init__(self, video_fps: float, collision_distance: float = 2.0,
                 ...
                 ttc_config: Optional[Dict] = None):
        """Initialize with enhanced TTC processing capabilities."""
    ...
    def calculate_ttc(self, tracker_id: int, kf_states: Dict,
        ...
        """
        Enhanced TTC calculation with filtering pipeline.
        ...
        # Fallback to legacy TTC calculation if enhanced data not available
